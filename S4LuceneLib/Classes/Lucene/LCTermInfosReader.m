...
- (LCTerm *) termAtPosition: (int)position
{
    ...
    LCSegmentTermEnumerator *enumerator = [self termEnumerator];
    if ((enumerator != nil) && ([enumerator term] != nil) && (position >= [enumerator position]) &&
		(position < ([enumerator position] + [enumerator indexInterval])))
	{
		return [self scanEnumeratorAtPosition: position];		  // can avoid seek
	}

	indexInterval = [enumerator indexInterval];
	if (0 < indexInterval)
	{
		[self seekEnumerator: (position / indexInterval)]; // must seek
	}
