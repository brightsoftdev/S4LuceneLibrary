 ...
 * @since   lucene 1.4
 * @version $Id: LCFieldCacheImpl.m 2722 2008-01-03 12:38:19Z mguesdon $
 */
...
- (NSDictionary *) strings: (LCIndexReader *) reader field: (NSString *) field
{
	...
	if (ret == nil) {
		...
		do {
			LCTerm *term = [termEnum term];
			if (term == nil || [[term field] isEqualToString: field] == NO) break;
			NSString *termval = AUTORELEASE([[term text] copy]);
			[termDocs seekTermEnumerator: termEnum];
			while ([termDocs hasNextDocument])
			{
				[retDic setObject: termval forKey: [NSNumber numberWithInt: [termDocs document]]];
			}
		} while ([termEnum hasNextTerm]);
		[termDocs close];
		[termEnum close];
						  ...
						  field: (NSString *) field
{
	...
	if (ret == nil) {
		...
		do {
			...
			[mterms addObject: [term text]];//FIXME Why autoreleasing it ?
